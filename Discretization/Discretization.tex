...
\left \| v \right \|_{V'} = \sup_{u\in V} \left \{ |v(u)|:\left \| u\right \|_V = 1 \right \}. 		\end{equation*}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\begin{equation}
				...
				\end{aligned}
			\end{equation}
			Where $\hat{f}:= f(F^{-1}(\hat{x}))$ and $\left \{ (\omega_k,\hat{p}_k) \right \}_k$ defines a \emph{quadrature rule}. This can be chosen in different ways, for higher order finite elements this may even affect the convergence behaviour. But for linear Lagrange elements, the trapezoidal rule works fine, i.e., using three points per element with appropriate weights.
		\end{enumerate}
	...
	\begin{theorem}[energy norm estimate, Knabner \cite{Knabner} page 144]\label{th:energy error estimate}
